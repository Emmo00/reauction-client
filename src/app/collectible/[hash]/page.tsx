...
export default function CollectiblePage({ params }: { params: { hash: string } }) {
  ...
  if (error) return <div>Error loading collectible</div>;
      ...
      <div className="min-h-screen bg-background pb-32">
        <div className="flex items-center justify-between px-4 py-4">
          ...
          <h1 className="text-base font-semibold">
            #{BigInt(hash).toString().substring(0, 6)} by {data.data.cast.author.display_name}
          </h1>
          <button className="flex h-10 w-10 items-center justify-center rounded-full bg-card"></button>
        </div>

        <div className="px-4 space-y-4">
          <CollectibleImage cast={data.data} size={340} className="mx-auto" />
          ...
          <Tabs defaultValue="details" className="w-full">
            ...
            <TabsContent value="details" className="mt-4">
              <div className="rounded-2xl bg-card p-4">
                ...
                <p className="text-sm text-muted-foreground">
                  A unique digital collectible featuring the iconic character with a halo. Part of
                  the exclusive EVOL collection, this NFT represents a rare piece of digital art.
                </p>
              </div>
            </TabsContent>
            <TabsContent value="owner" className="mt-4">
              <div className="rounded-2xl bg-card p-4">
                <p className="text-sm text-muted-foreground">
                  Owner information will be displayed here.
                </p>
              </div>
            </TabsContent>
